...
//    static ArrayList<Bullet>[] activeBullets = new ArrayList<>();
//    static activeBullets[]
    public static ArrayList<Bullet>[] activeBullets;

    static {
        activeBullets = new ArrayList[CONST.Tank.NUM_ENEMIES];
        for(int i = 0; i < activeBullets.length; i++) {
            activeBullets[i] = new ArrayList<>();
        }
    ...
    public Enemy(ObjectType type, int x, int y) {
        ...
        id = nxtId;
        nxtId++;
//        Log.d("NEW ENEMY: ", String.valueOf(id));
//        EnemyCount++;
        target = new Point();
        ...
        activeBullets[id] = new ArrayList<>();
//        moving = true;
    }

    public Enemy(ObjectType type, int group, int x, int y, int eid) {
        super(type, x, y, 0);
        id = eid;
        nxtId = id+1;
//        Log.d("NEW ENEMY: ", String.valueOf(id));
...
//    }
    ...
    public void fire() {
        ...
        bullet.id = bId;
        //todo add enemy bullet to tank game bullets
        bullet.setBreakWall(breakWall);
//        bullets.add((new Bullet(bullet,bx,by)));
        activeBullets[id].add(new Bullet(bullet,bx,by));
...
//        while(bullets.remove(null));
        ...
        //todo -- commented bullet move update
//        for(Bullet bullet:bullets) {
//            bullet.move();
//        }
    }

    // todo update active bullets
    public static void updateActiveBullets() {
        for(int i = 0; i < activeBullets.length; i++) {
            if(activeBullets[i] == null) {
                continue;
            }
            for(int j = 0; j < activeBullets[i].size(); j++) {
                if (activeBullets[i].get(j).recycle) {
                    activeBullets[i].set(j, null);
                }
            }
        }

        for (ArrayList<Bullet> activeBullet : activeBullets) {
...
//                respawn();
        ...
        // todo commented draw personal bullet
//        for(Bullet bullet:bullets) {
//            if(bullet != null) {
//                bullet.draw(canvas);
//            }
