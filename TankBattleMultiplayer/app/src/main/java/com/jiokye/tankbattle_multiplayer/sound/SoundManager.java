package com.jiokye.tankbattle_multiplayer.sound;
...
//            activeSounds.set(index,true);
    ...
    {
        if(activeSounds == null || index < 0 || index >= activeSounds.size()) {
            return;
        }
    ...
    {
        if(activeSounds == null || (loop && activeSounds.get(index))) {
            return;
        }
...
//        Log.d("RESUME", "Resuming game sounds");
    ...
    public static void resumeSound(int index) {
        if(!playSound || activeSounds == null || index < 0 || index >= activeSounds.size()) {
            return;
        }
...
//        Log.d("STOP", "Stopping game sounds");
    ...
    public static boolean isActive(int index) {
        ...
        if(index < activeSounds.size() && index >= 0) {
