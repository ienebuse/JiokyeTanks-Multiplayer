...
# Multiplayer lobby - create or join a room via room code or IP address
...
var connected_peer_id: int = -1

func _ready() -> void:
	start_btn.visible = false
	status_label.text = ""
	ip_label.text = "Your IP: " + NetworkManager.get_local_ip()
	ip_input.placeholder_text = "Room code or IP address"
...
func _on_join_btn_pressed() -> void:
	var input_text = ip_input.text.strip_edges().to_upper()
	if input_text.is_empty():
		status_label.text = "Please enter a room code or IP address"
		return
	
	var address: String = ""
	if NetworkManager.is_room_code(input_text):
		status_label.text = "Resolving room code " + input_text + "..."
		address = NetworkManager.resolve_room_code(input_text)
		if address.is_empty():
			status_label.text = "Could not find room with code: " + input_text + "\nMake sure you're on the same WiFi network."
			return
		status_label.text = "Found host at " + address + ", connecting..."
	else:
		address = input_text
...
func _on_connection_failed() -> void:
	status_label.text = "Connection failed. Check room code/IP and try again."
	create_btn.disabled = false
	join_btn.disabled = false
...
func _on_start_btn_pressed() -> void:
	if not peer_connected:
		return
	_remote_start_game.rpc_id(connected_peer_id)
	_start_game()
