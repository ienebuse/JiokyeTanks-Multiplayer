...
func play_sound(sound_name: String, loop: bool = false, volume_db: float = 0.0) -> void:
	if not sound_enabled:
		return
	# Stop existing instance of this sound if already playing
	if active_players.has(sound_name):
		var existing = active_players[sound_name]
		if is_instance_valid(existing):
			existing.set_meta("looping", false)
			existing.stop()
			existing.queue_free()
...
func pause_sound(sound_name: String) -> void:
	if active_players.has(sound_name):
		var player = active_players[sound_name]
		if is_instance_valid(player) and player.playing:
			player.stream_paused = true

func resume_sound(sound_name: String) -> void:
	if not sound_enabled:
		return
	if active_players.has(sound_name):
		var player = active_players[sound_name]
